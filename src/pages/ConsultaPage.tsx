...
export default function ConsultaPage() {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        const [u, h] = await Promise.all([obtenerUltimoCorte(), obtenerHistorialCortes()])
        setUltimo(u)
        setHistorial(h)
      } catch (err) {
    ...
    <AppShell titulo="Consulta de resultados" ancho="max-w-4xl">
      ...
      {ultimo === null && !error && (
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          <p className="text-slate-600">Aún no se ha publicado ningún corte de resultados.</p>
        </div>
      )}
        ...
        <>
          ...
          <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Casillas</p>
              <p className="mt-1 text-2xl font-semibold text-impepac-ink">{ultimo.totalCasillas}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Capturadas</p>
              <p className="mt-1 text-2xl font-semibold text-impepac-ink">{ultimo.totalActasCapturadas}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Validadas</p>
              <p className="mt-1 text-2xl font-semibold text-impepac-ink">{ultimo.totalActasValidadas}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Participación</p>
              <p className="mt-1 text-2xl font-semibold text-impepac-ink">
                ...
                return (
                  <div key={clave}>
                    ...
                    <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                      <div
                        className="h-full rounded-full bg-impepac-magenta-600"
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
